...
function parseCSV(text) {
  ...
  const required = ['english', 'vietnamese', 'examplesentence', 'englishmeaning', 'difficulty', 'category'];
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      if (!obj.english || !obj.vietnamese) { errorRows++; continue; }
...
async function importCSV(text) {
  ...
  if (!parsed) {
    showImportResult('Định dạng CSV không hợp lệ. Vui lòng kiểm tra lại file.', 'error');
    return;
  }
  ...
  try {
    const res = await chrome.runtime.sendMessage({ type: 'IMPORT_WORDS', words: parsed.words });
    if (!res?.ok) {
...
function difficultyClass(d) {
  return { B1: 'badge-b1', B2: 'badge-b2', C1: 'badge-c1', C2: 'badge-c2' }[d] || 'badge-b1';
}
...
async function loadLearningStatus() {
  try {
    const status = await chrome.runtime.sendMessage({ type: 'GET_LEARNING_STATUS' });
    ...
    if (todayEl) {
      ...
      const pct = status.todayTotal > 0 ? (status.todayStudied / status.todayTotal) * 100 : 0;
...
document.getElementById('reset-stats-btn').addEventListener('click', async () => {
  if (!confirm('Đặt lại tất cả thống kê và tiến độ học?\nThao tác này không thể hoàn tác.')) return;
...
function renderSettings() {
  ...
  document.querySelectorAll('#direction-control .seg-btn').forEach(b =>
    b.classList.toggle('active', b.dataset.value === (settings.questionDirection || 'mixed')));

  document.querySelectorAll('#answer-type-control .seg-btn').forEach(b =>
    b.classList.toggle('active', b.dataset.value === (settings.answerType || 'choice')));

  const selDiff = settings.selectedDifficulties || [];
  document.querySelectorAll('#difficulty-filter .filter-pill').forEach(b =>
    b.classList.toggle('active', selDiff.includes(b.dataset.value)));

  document.querySelectorAll('.timeout-btn').forEach(b =>
    b.classList.toggle('active', parseInt(b.dataset.value) === (settings.sessionTimeoutSeconds ?? 90)));
}
...
document.getElementById('enabled-toggle').addEventListener('change', async e => {
  ...
  document.getElementById('enabled-toggle-settings').checked = e.target.checked;
  await saveSettings();
});
document.getElementById('enabled-toggle-settings').addEventListener('change', async e => {
  settings.enabled = e.target.checked;
...
document.querySelectorAll('.popup-size-btn').forEach(btn => {
  btn.addEventListener('click', async () => {
    ...
    document.querySelectorAll('.popup-size-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    await saveSettings();
  });
});
...
document.getElementById('wpd-dec').addEventListener('click', async () => {
  ...
  input.value = v; settings.wordsPerDay = v; await saveSettings();
});
...
document.getElementById('words-per-day').addEventListener('change', async e => {
  const v = Math.max(1, Math.min(20, parseInt(e.target.value) || 5));
  e.target.value = v; settings.wordsPerDay = v; await saveSettings();
});
...
document.querySelectorAll('#direction-control .seg-btn').forEach(btn => {
  btn.addEventListener('click', async () => {
    ...
    document.querySelectorAll('#direction-control .seg-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    await saveSettings();
  });
});
...
document.querySelectorAll('#answer-type-control .seg-btn').forEach(btn => {
  btn.addEventListener('click', async () => {
    ...
    document.querySelectorAll('#answer-type-control .seg-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    await saveSettings();
  });
});
...
// Clear all words
document.getElementById('clear-words-btn').addEventListener('click', async () => {
...
document.getElementById('reset-default-btn').addEventListener('click', async () => {
  if (!confirm('Khôi phục danh sách từ mặc định?\nToàn bộ từ hiện tại và tiến độ sẽ bị xóa.')) return;
...
document.getElementById('add-word-btn').addEventListener('click', async () => {
  ...
  if (exists) {
    ...
    setTimeout(() => { engInput.style.borderColor = ''; }, 1500);
    return;
  }

  const res = await chrome.runtime.sendMessage({
    type: 'IMPORT_WORDS',
    words: [{ english, vietnamese, exampleSentence: '', englishMeaning: '', difficulty: 'B1', category: 'general' }]
  });
  await loadData();
  engInput.value = ''; vnInput.value = '';
...
function renderAllWords() {
  ...
  const pageWords = filtered.slice(awPage * AW_PAGE_SIZE, (awPage + 1) * AW_PAGE_SIZE);
  ...
  list.innerHTML = pageWords.length === 0
    ? '<div class="empty-msg">Không có từ nào</div>'
    : pageWords.map(w => `
        <div class="word-item">
          <div class="word-main">
  ...
  if (prevBtn) prevBtn.addEventListener('click', () => { awPage--; renderAllWords(); });
  if (nextBtn) nextBtn.addEventListener('click', () => { awPage++; renderAllWords(); });
}

document.getElementById('aw-search').addEventListener('input', e => {
...
function startCountdown() {
  ...
  const tick = async () => {
    try {
      const { scheduledTime } = await chrome.runtime.sendMessage({ type: 'GET_NEXT_ALARM' });
      const el = document.getElementById('countdown');
      if (!el) return;
      if (!scheduledTime) { el.textContent = '--:--'; return; }
      const r = scheduledTime - Date.now();
      if (r <= 0) { el.textContent = '00:00'; return; }
